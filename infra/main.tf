...
locals {
  function_get_notes   = "get-notes"
  function_save_note   = "save-note"
  function_delete_note = "delete-note"
  ...
  "Statement": [
    {
      ...
      "Resource": ["arn:aws:logs:*:*:*", "${aws_dynamodb_table.notes-application.arn}"],
      "Effect": "Allow"
    }
  ]
}
...
# dynamodb table
resource "aws_dynamodb_table" "notes-application" {
